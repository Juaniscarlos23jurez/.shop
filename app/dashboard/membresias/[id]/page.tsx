...
import { 
  Plus, X, Check, Loader2, Package, DollarSign, Trash2, 
...
export default function EditMembershipPage() {
    ...
    const fetchMembershipData = async () => {
      try {
        ...
        if (!companyResponse.success || !companyResponse.data?.data?.id) {
          throw new Error('No se encontró la compañía asociada a tu cuenta');
        }
        const companyId = companyResponse.data.data.id;
        
        // 2. Get membership detail
        const response = await api.memberships.getMembership(companyId.toString(), id as string, token);
        
        if (!response.success || !response.data?.data) {
          throw new Error(response.message || 'Error al cargar la membresía');
        }
        ...
        setFormData({
          ...
          welcomeGift: m.welcome_gift,
          birthdayGift: m.birthday_gift,
          earlyRenewalDiscount: m.early_renewal_discount?.toString() || '',
          maxUsers: m.max_users?.toString() || '',
          validFrom: m.valid_from ? m.valid_from.split(' ')[0] : '',
          validUntil: m.valid_until ? m.valid_until.split(' ')[0] : '',
          imageUrl: m.image_url || '',
          backgroundImageUrl: m.background_image_url || '',
          cardColorHex: m.card_color_hex || '#ffffff',
          accentColorHex: m.accent_color_hex || '#10b981',
          textColorHex: m.text_color_hex || '#0f172a',
        ...
        if (m.background_image_url) setBgPreview(m.background_image_url);
        
        setBenefits(m.benefits.map((b, index) => ({
          id: index.toString(),
          text: b.text,
          type: (b.type as ValidBenefitType) || 'custom',
  ...
  const uploadFile = async (file: File, type: 'logo' | 'background', mId: string) => {
    const path = `memberships/${mId}/${type}/${Date.now()}_${file.name}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedImageFile) {
        finalImageUrl = await uploadFile(selectedImageFile, 'logo', id as string);
      }
      if (selectedBgFile) {
        finalBgUrl = await uploadFile(selectedBgFile, 'background', id as string);
      }
      ...
      const response = await api.memberships.updateMembership(companyId.toString(), id as string, membershipData, token);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await api.memberships.deleteMembership(companyId.toString(), id as string, token);
      ...
      setError(err instanceof Error ? err.message : 'Error al eliminar la membresía');
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Actualiza los detalles y la apariencia de tu plan</p>
        </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input id="price" name="price" type="number" step="0.01" className="pl-8" value={formData.price} onChange={handleInputChange} required />
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2">
                ...
                <CardContent className="space-y-6">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="earlyRenewalDiscount" name="earlyRenewalDiscount" type="number" value={formData.earlyRenewalDiscount} onChange={handleInputChange} />
                    </div>
                  </div>
